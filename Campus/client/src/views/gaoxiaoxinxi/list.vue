<template>
        ...
        <div class="form-search">
            <el-form @submit.prevent.stop :inline="true" size="mini">
                ...
                </el-form-item>                <el-form-item label="高校名称">
                
                    <el-input v-model="search.gaoxiaomingcheng"></el-input>
            ...
            <el-table-column type="index" label="#"></el-table-column> <!-- 序号 -->
                ...
                <template slot-scope="{row}">
                    {{  row.bianhao  }}                </template>
                        ...
                        <el-table-column label="高校名称">
                <template slot-scope="{row}">
                    {{  row.gaoxiaomingcheng  }}                </template>
            </el-table-column>
                        <el-table-column label="图片"width="100">
                <template slot-scope="{row}">
                    <e-img :src="row.tupian" style="max-width:120px" />                </template>
            </el-table-column>
                        <el-table-column label="高校地址">
                <template slot-scope="{row}">
                    {{  row.gaoxiaodizhi  }}                </template>
            </el-table-column>
                        <el-table-column label="高校负责人"width="130">
                <template slot-scope="{row}">
                    {{  row.gaoxiaofuzeren  }}                </template>
            </el-table-column>
                ...
                <template slot-scope="{row}">
                    {{ row.issh }}
                                        <el-button size="mini" :type="row.issh == '是' ? 'success' : 'danger'" @click="checkIssh(row,'gaoxiaoxinxi')">
                        {{ row.issh == '是' ? '取消审核' : '审核' }}
                    </el-button>
                                    </template>
            </el-table-column>
            ...
            <el-table-column label="操作">
                <template slot-scope="{row}">
                    <el-button-group>
                        ...
                        <el-button @click="$goto({path:'/admin/gaoxiaoxinxidetail',query:{id:row.id } })" icon="el-icon-info" type="info" size="mini"></el-button>
                        </el-tooltip>
                                            <el-tooltip content="编辑" placement="top">
                    <el-button icon="el-icon-edit" @click="$goto({path:'/admin/gaoxiaoxinxiupdt',query:{id:row.id } })"
    ...
    export default {
        ...
        methods: {
            ...
            loadList( page ){
                ...
                this.$post(api.gaoxiaoxinxi.list , filter).then(res=>{
                    this.loading = false;
            ...
            deleteItem( row ){
                this.$confirm('确定删除数据？' , '提示',{ // 弹出 确认框提示是否要删除
                    ...
                    this.$post(api.gaoxiaoxinxi.delete , {// 提交后台
                        id:row.id
                    }).then(res=>{
                        this.loading = false;
                        if(res.code != api.code.OK){
