    ...
    <div class="v-list" v-loading="loading" element-loading-text="加载中">
        ...
        <el-card class="box-card">
            ...
            >
                ...
                <el-table-column label="答题时长" width="80">
                    <template slot-scope="{row}">
                        {{ row.datishichang }}
                    </template>
                </el-table-column>
                <el-table-column label="类型" width="80">
                    <template slot-scope="{row}">
                        <e-select-view module="leixing" :value="row.leixing" select="id" show="leixing"></e-select-view>
                    </template>
                </el-table-column>
                ...
                <el-table-column label="操作">
                    <template slot-scope="{row}">
                        <el-button-group>
                            <el-button @click="$goto('/admin/shitiadd?id='+row.id)" type="success" icon="el-icon-plus"
                                       size="mini">
                                试题添加
                            </el-button>
                            <el-tooltip content="详情" placement="top">
                                <el-button @click="$goto({path:'/admin/diaochawenjuandetail',query:{id:row.id } })"
                                           icon="el-icon-info" type="info" size="mini"></el-button>
                            </el-tooltip>
                            <el-tooltip content="编辑" placement="top">
                                <el-button icon="el-icon-edit"
                                           @click="$goto({path:'/admin/diaochawenjuanupdt',query:{id:row.id } })"
    ...
    export default {
        ...
        methods: {
            ...
            loadList(page) {
                ...
                this.$post(api.diaochawenjuan.listfaburen, filter).then(res => {
                    this.loading = false;
            ...
            deleteItem(row) {
                this.$confirm('确定删除数据？', '提示', { // 弹出 确认框提示是否要删除
                    ...
                    this.$post(api.diaochawenjuan.delete, {// 提交后台
                        id: row.id
                    }).then(res => {
                        this.loading = false;
                        if (res.code != api.code.OK) {
