<template>
        ...
        <div class="form-search">
            <el-form @submit.prevent.stop :inline="true" size="mini">
                ...
                </el-form-item>                <el-form-item label="考试编号">
                
                    <el-input v-model="search.kaoshibianhao"></el-input>
                    
                </el-form-item>                <el-form-item label="考试时长">
                
                    <el-input v-model="search.kaoshishizhang"></el-input>
            ...
            <el-table-column type="index" label="#"></el-table-column> <!-- 序号 -->
                ...
                <template slot-scope="{row}">
                    {{  row.faburen  }}                </template>
                        ...
                        <el-table-column label="考试编号"width="130">
                <template slot-scope="{row}">
                    {{  row.kaoshibianhao  }}                </template>
            </el-table-column>
                        <el-table-column label="考试时长"width="130">
                <template slot-scope="{row}">
                    {{  row.kaoshishizhang  }}                </template>
            </el-table-column>
                        <el-table-column label="总得分"width="80">
                <template slot-scope="{row}">
                    {{  row.zongdefen  }}                </template>
            </el-table-column>
                        <el-table-column label="考试人"width="80">
                <template slot-scope="{row}">
            ...
            <el-table-column label="操作">
                <template slot-scope="{row}">
                    <el-button-group>
                        ...
                        <el-button @click="$goto({path:'/admin/ceshichengjidetail',query:{id:row.id } })" icon="el-icon-info" type="info" size="mini"></el-button>
                        </el-tooltip>
                                            <el-tooltip content="编辑" placement="top">
                    <el-button icon="el-icon-edit" @click="$goto({path:'/admin/ceshichengjiupdt',query:{id:row.id } })"
    ...
    export default {
        ...
        methods: {
            ...
            loadList( page ){
                ...
                this.$post(api.ceshichengji.listkaoshiren , filter).then(res=>{
                    this.loading = false;
            ...
            deleteItem( row ){
                this.$confirm('确定删除数据？' , '提示',{ // 弹出 确认框提示是否要删除
                    ...
                    this.$post(api.ceshichengji.delete , {// 提交后台
                        id:row.id
                    }).then(res=>{
                        this.loading = false;
                        if(res.code != api.code.OK){
