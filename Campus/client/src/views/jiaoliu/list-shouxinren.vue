    ...
    <div class="v-list" v-loading="loading" element-loading-text="加载中">
        ...
        <el-card class="box-card">
            ...
            >
                ...
                <el-table-column label="收信人" width="80">
                    <template slot-scope="{row}">
                        {{ row.shouxinren }}
                    </template>
                </el-table-column>
                ...
                <el-table-column label="操作">
                    <template slot-scope="{row}">
                        <el-button-group>
                            <el-button @click="$goto('/admin/liaotianjiluadd?id='+row.id)" type="success"
                                       icon="el-icon-plus" size="mini">
                                聊天记录
                            </el-button>
                            <el-tooltip content="编辑" placement="top">
                                <el-button icon="el-icon-edit"
                                           @click="$goto({path:'/admin/jiaoliuupdt',query:{id:row.id } })"
    ...
    export default {
        ...
        methods: {
            ...
            loadList(page) {
                ...
                this.$post(api.jiaoliu.listshouxinren, filter).then(res => {
                    this.loading = false;
            ...
            deleteItem(row) {
                this.$confirm('确定删除数据？', '提示', { // 弹出 确认框提示是否要删除
                    ...
                    this.$post(api.jiaoliu.delete, {// 提交后台
                        id: row.id
                    }).then(res => {
                        this.loading = false;
                        if (res.code != api.code.OK) {
