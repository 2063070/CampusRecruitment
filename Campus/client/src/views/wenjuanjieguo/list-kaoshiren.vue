<template>
        ...
        <div class="form-search">
            <el-form @submit.prevent.stop :inline="true" size="mini">
                ...
                </el-form-item>                <el-form-item label="答题编号">
                
                    <el-input v-model="search.datibianhao"></el-input>
        ...
        <el-table border :data="list" style="width: 100%" highlight-current-row
            ...
            <el-table-column type="index" label="#"></el-table-column> <!-- 序号 -->
                ...
                <template slot-scope="{row}">
                    {{  row.mingcheng  }}                </template>
            </el-table-column>
                        <el-table-column label="发布人"width="80">
                <template slot-scope="{row}">
                    {{  row.faburen  }}                </template>
            </el-table-column>
                        <el-table-column label="答题编号"width="130">
                <template slot-scope="{row}">
                    {{  row.datibianhao  }}                </template>
            </el-table-column>
                        <el-table-column label="答题时长"width="130">
                <template slot-scope="{row}">
                    {{  row.datishichang  }}                </template>
            </el-table-column>
                        <el-table-column label="总得分"width="80">
                <template slot-scope="{row}">
                    {{  row.zongdefen  }}                </template>
            </el-table-column>
                        <el-table-column label="答题人"width="80">
                <template slot-scope="{row}">
                    {{  row.datiren  }}                </template>
            </el-table-column>
            ...
            <el-table-column label="操作">
                <template slot-scope="{row}">
                    <el-button-group>
                        ...
                        <el-button @click="$goto({path:'/admin/wenjuanjieguodetail',query:{id:row.id } })" icon="el-icon-info" type="info" size="mini"></el-button>
                        </el-tooltip>
                                            <el-tooltip content="编辑" placement="top">
                    <el-button icon="el-icon-edit" @click="$goto({path:'/admin/wenjuanjieguoupdt',query:{id:row.id } })"
    ...
    export default {
        ...
        methods: {
            ...
            loadList( page ){
                ...
                this.$post(api.wenjuanjieguo.listkaoshiren , filter).then(res=>{
                    this.loading = false;
            ...
            deleteItem( row ){
                this.$confirm('确定删除数据？' , '提示',{ // 弹出 确认框提示是否要删除
                    ...
                    this.$post(api.wenjuanjieguo.delete , {// 提交后台
                        id:row.id
                    }).then(res=>{
                        this.loading = false;
                        if(res.code != api.code.OK){
