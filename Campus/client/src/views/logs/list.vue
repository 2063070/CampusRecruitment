<template>
        ...
        <div class="form-search">
            <el-form @submit.prevent.stop :inline="true" size="mini">
                    ...
                    <el-input v-model="search.module"></el-input>
                    
                </el-form-item>                <el-form-item label="操作类型">
                
                    <el-select v-model="search.operationtype" ><el-option label="请选择" value=""></el-option></el-select>
                    
                </el-form-item>                <el-form-item label="操作内容">
        ...
        <el-table border :data="list" style="width: 100%" highlight-current-row
            ...
            <el-table-column type="index" label="#"></el-table-column> <!-- 序号 -->
                ...
                <template slot-scope="{row}">
                    {{  row.operationtype  }}                </template>
            </el-table-column>
                        <el-table-column label="操作内容">
                <template slot-scope="{row}">
                    {{  row.operationcontent  }}                </template>
            </el-table-column>
                        <el-table-column label="角色"width="130">
                <template slot-scope="{row}">
                    {{  row.cx  }}                </template>
            </el-table-column>
                        <el-table-column label="操作用户"width="80">
                <template slot-scope="{row}">
            ...
            <el-table-column label="操作">
                <template slot-scope="{row}">
                    ...
                    <el-button icon="el-icon-edit" @click="$goto({path:'/admin/logsupdt',query:{id:row.id } })"
        ...
        methods: {
            ...
            batchDelete(){
                ...
                this.$confirm('确定删除？').then(res=>{
                    ...
                    var ids = this.multipleSelection.map(o=>o.id);//获取所有选中行的id
                    this.loading = true; // 滚动条
                    this.$post(api.logs.batch, {// 提交后台
                        delete:1,
                        ids: ids
                    }).then(res => {
                        this.loading = false;
            ...
            loadList( page ){
                ...
                this.$post(api.logs.list , filter).then(res=>{
                    this.loading = false;
                    if(res.code == api.code.OK)
                    {
            ...
            deleteItem( row ){
                this.$confirm('确定删除数据？' , '提示',{ // 弹出 确认框提示是否要删除
                    ...
                    this.loading = true; // 滚动条
                    this.$post(api.logs.delete , {// 提交后台
                        id:row.id
                    }).then(res=>{
                        this.loading = false;
                        if(res.code != api.code.OK){
