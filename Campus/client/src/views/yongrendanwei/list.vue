        ...
        <el-table border :data="list" style="width: 100%" highlight-current-row
            ...
            <el-table-column type="index" label="#"></el-table-column> <!-- 序号 -->
                ...
                <template slot-scope="{row}">
                    {{  row.zhanghao  }}                </template>
            </el-table-column>
                        <el-table-column label="单位名称">
                <template slot-scope="{row}">
                    {{  row.danweimingcheng  }}                </template>
            </el-table-column>
                        <el-table-column label="负责人"width="130">
                <template slot-scope="{row}">
                    {{  row.fuzeren  }}                </template>
            </el-table-column>
                ...
                <template slot-scope="{row}">
                    {{  row.danweidizhi  }}                </template>
            </el-table-column>
                        <el-table-column label="资质证明"width="100">
                <template slot-scope="{row}">
                    <e-img :src="row.zizhizhengming" style="max-width:120px" />                </template>
            </el-table-column>
                ...
                <template slot-scope="{row}">
                    {{ row.issh }}
                                        <el-button size="mini" :type="row.issh == '是' ? 'success' : 'danger'" @click="checkIssh(row,'yongrendanwei')">
                        {{ row.issh == '是' ? '取消审核' : '审核' }}
                    </el-button>
                                    </template>
            </el-table-column>
            ...
            <el-table-column label="操作">
                <template slot-scope="{row}">
                    ...
                    <el-button icon="el-icon-edit" @click="$goto({path:'/admin/yongrendanweiupdt',query:{id:row.id } })"
    ...
    export default {
        ...
        methods: {
            ...
            loadList( page ){
                ...
                this.$post(api.yongrendanwei.list , filter).then(res=>{
                    this.loading = false;
            ...
            deleteItem( row ){
                this.$confirm('确定删除数据？' , '提示',{ // 弹出 确认框提示是否要删除
                    ...
                    this.$post(api.yongrendanwei.delete , {// 提交后台
                        id:row.id
                    }).then(res=>{
                        this.loading = false;
                        if(res.code != api.code.OK){
